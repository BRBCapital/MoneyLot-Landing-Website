...
const CurlyTickerMobile: React.FC<CurlyTickerProps> = ({
  ...
  useEffect(() => {
    ...
    const setupTicker = async () => {
      ...
      if (isSafari) {
        ...
        tPath2.setAttribute("startOffset", String(xRef.current + maxLen + 10));
      } else {
        xRef.current = -T;
        tPath1.setAttribute("startOffset", String(xRef.current));
    ...
    <div className="absolute bottom-0 left-0 w-full overflow-hidden pointer-events-none select-none z-50">
      ...
      >
        ...
        <text
          ...
          dominantBaseline="middle"
        >
          <textPath ref={textPath2Ref} href="#ribbonPathMobile">
            {phrase.repeat(3)}
          </textPath>
        </text>
      </svg>
...
export default CurlyTickerMobile;
